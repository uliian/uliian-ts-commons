...
export function findInTreeFirst<T extends IMultiTreeItem<T>>(
    ...
    for (let item of tree) {
        ...
        if (item.children !== null && item.children!.length > 0) {
            let result = findInTreeFirst(item.children, condition);
            if (result) {
                return result;
            }
        }
...
export function treeMap<TOut extends IMultiTreeItem<TOut>, TIn extends IMultiTreeItem<TIn>>(
    ...
    for (let item of tree) {
        ...
        result.push(newItem);
        if (item.children !== null && item.children!.length > 0) {
            let childrenResult = treeMap(item.children, mapFun);
...
export function treeFilter<TIn>(
    ...
    for (let item of tree) {
        ...
        if ((childrenSelct(item)?.length ?? 0) > 0) {
            let childrenResult = treeFilter(childrenSelct(item) ?? [], childrenSelct, childrenSet, condition);
